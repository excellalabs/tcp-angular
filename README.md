...
2a) Generating AWS credentials
Generate credentials with awsmfa (may need to do on your own laptop)
Copy those credentials to the computer you're working on: ~/.aws/credentials
...
2b) Configuring AWS command-line tool
Run:
`aws-configure`
...
2c) Configureing `ecs-cli` command-line tool
Run:
...
8a) Create a new target group; make its name end with "-to-3000-tg".
...
8b) Go to the cluster node ASG (name contains "-cluster-node") and attach the new target group:
"Details" tab, "Edit" button.  Click in the "Target Groups" box, type "3000", select the new target group.
Click "Save".

8c) Remove any exsiting Listeners on the ALB that are listening on port 80.
Add a Listener to the ALB: listen on 80, forward to the new target group.

8d) Make sure the ALB's Security Group (name ends with "-alb-sg") allows connections on port 80.
8e) Set the cluster node security group (name ends with "-cluster-instance-sg") to
